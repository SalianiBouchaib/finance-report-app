...
def generate_pdf_report(report_name, sections):
    ...
    def generate_additional_charts():
        ...
        # 3. Tableau de trésorerie mensuel
        if has_data('monthly_cashflow_data'):
            try:
                ...
                ax.bar(x + width/2, depenses, width, label='Dépenses')
...
Adresse: {ascii_only(basic_info.get('address', 'Non specifie'))}
            ...
            try:
                ...
                if isinstance(df_csv, pd.DataFrame) and not df_csv.empty:
                    ...
                    if hasattr(st.session_state, "metrics") and st.session_state.metrics:
                        ...
                        # Ajouter un graphique de répartition si possible
                        try:
                            ...
                            logger.error(f"Erreur graphique répartition: {e}")
                            pdf.chapter_body(f"Erreur lors de la generation du graphique: {ascii_only(str(e))}")
                    
                    # Ajout de graphiques supplémentaires à la section CSV
                    pdf.add_page()
                    pdf.chapter_title("Analyses graphiques des données CSV")
                    
                    # Graphique 1: Distribution des valeurs numériques
                    try:
                        numeric_cols = df_csv.select_dtypes(include=['int64', 'float64']).columns.tolist()
                        if len(numeric_cols) > 0 and len(numeric_cols) <= 5:  # Limiter à 5 colonnes pour la lisibilité
                            fig, ax = plt.subplots(figsize=(10, 6))
                            for col in numeric_cols:
                                if not df_csv[col].isnull().all():  # Vérifier que la colonne n'est pas entièrement nulle
                        ...
                        logger.error(f"Erreur distribution numérique: {e}")
                        pdf.chapter_body(f"Erreur distribution numérique: {ascii_only(str(e))}")
                    
                    # Graphique 2: Matrice de corrélation
                    try:
                        numeric_cols = df_csv.select_dtypes(include=['int64', 'float64']).columns.tolist()
                        if len(numeric_cols) >= 2:  # Au moins 2 colonnes pour une corrélation
                            ...
                            if img_path:
                                pdf.ln(10)
                                pdf.add_image(img_path, w=180, caption="Matrice de corrélation des variables")
                    except Exception as e:
                        logger.error(f"Erreur matrice de corrélation: {e}")
                        pdf.chapter_body(f"Erreur matrice de corrélation: {ascii_only(str(e))}")
                    
                    # Graphique 3: Analyse temporelle si une colonne de date est présente
                    try:
                        ...
                        if date_cols and numeric_cols:
                            ...
                            if not pd.api.types.is_datetime64_dtype(df_csv[date_col]):
                                ...
                                try:
                                    ...
                                    ax.set_title(f'Évolution de {numeric_col} dans le temps')
                                    fig.autofmt_xdate()  # Pour une meilleure rotation des dates
                                    
                                    img_path = save_figure_safely(fig, "csv_time_series.png", temp_dir)
                                    if img_path:
                                        pdf.ln(10)
                                        pdf.add_image(img_path, w=180, caption=f"Évolution de {numeric_col} dans le temps")
        ...
        if "Cash Flow" in sections and (has_data("cash_flow_data") or hasattr(st.session_state, "calculated_data")):
            ...
            # Ajouter les graphiques existants de cash flow dans le répertoire temporaire
            try:
                # Générer un graphique de flux de trésorerie
                if hasattr(st.session_state, "calculated_data") and hasattr(st.session_state.calculated_data, "cashflow_years"):
                    try:
                        ...
                        if isinstance(cashflow_years, list) and isinstance(cashflow_values, list) and len(cashflow_years) == len(cashflow_values):
                            ...
                            ax.set_xlabel('Années')
                            ax.set_ylabel('Flux de trésorerie (DHS)')
                            ax.set_title('Évolution des flux de trésorerie')
                            ax.grid(True, linestyle='--', alpha=0.7)
                            
                            img_path = save_figure_safely(fig, "cashflow_evolution.png", temp_dir)
                            if img_path:
                                pdf.ln(10)
                                pdf.add_image(img_path, w=180, caption="Évolution des flux de trésorerie")
                    except Exception as e:
                        logger.error(f"Erreur lors de la création du graphique de flux de trésorerie: {e}")
            except Exception as e:
                logger.error(f"Erreur lors de l'ajout des graphiques de cash flow: {e}")
            
            # Ajout des détails par catégorie dans la section Cash Flow
            if has_data("cash_flow_categories") or hasattr(st.session_state, "cash_flow_detailed"):
                    ...
                    for category, data in cf_categories.items():
                        ...
                        if isinstance(data, dict):
                            ...
                            try:
                                ...
                                # En cas d'erreur, calculer différemment
                                try:
                ...
                try:
                    ...
                    if categories and any(v > 0 for v in categories.values()):
                        ...
                        ax.set_ylabel('Montant (DHS)')
                        ax.set_title('Répartition du cash flow par catégorie')
        ...
        if "Amortissements" in sections:
            ...
            if has_data("credits"):
                ...
                for i, credit in enumerate(st.session_state.credits):
                    ...
                    # Calcul de l'amortissement simplifié
                    if principal > 0 and rate > 0 and term > 0:
                        try:
                            ...
                            # Création d'un tableau d'amortissement résumé
                            headers = ["Annee", "Paiement", "Capital", "Interets", "Solde"]
                            table_data = []
                            balance = principal
                            
                            for year in range(1, min(term + 1, 6)):  # Limiter à 5 ans pour le PDF
                            ...
                            try:
                                # Calcul simplifié sur toute la durée
                                ...
                                ax.pie([principal, total_interest], 
                                      ...
                                      startangle=90)
                                ax.axis('equal')
                                plt.title(f"Répartition Capital/Intérêts pour {credit.get('Nom', f'Credit {i+1}')}")
                                
                                img_path = save_figure_safely(fig, f"credit_{i}_pie.png", temp_dir)
                                if img_path:
                                    pdf.ln(10)
                                    pdf.add_image(img_path, w=150, caption="Répartition Capital/Intérêts")
                            except Exception as e:
                        ...
                        pdf.chapter_body("Données de crédit insuffisantes pour calculer l'amortissement.")
                    
                    # Ajouter un saut de page entre les crédits, sauf pour le dernier
                    if i < len(st.session_state.credits) - 1:
                        pdf.add_page()
            else:
                pdf.chapter_body("Aucun crédit n'a été ajouté dans la section Investissements.")
            
            # Ajout d'un résumé global des amortissements
            if has_data("amortization_summary"):
                pdf.ln(10)
                pdf.chapter_title("Résumé Global des Amortissements")
                
                summary = st.session_state.amortization_summary
                headers = ["Année", "Amortissement Total", "Valeur Nette Comptable"]
                table_data = []
                ...
                if table_data:  # Vérifier qu'il y a des données à afficher
                    ...
                    try:
                        ...
                        for row in table_data:
                            try:
                                years.append(row[0])
                                # Extraire la valeur numérique en enlevant la formatation
                        ...
                        if years and amort_values and len(years) == len(amort_values):
                            ...
                            ax.set_xlabel('Années')
                            ax.set_ylabel('Montant (DHS)')
                            ax.set_title('Évolution des amortissements et valeurs nettes')
                            ax.set_xticks(x)
                            ax.set_xticklabels(years)
                            ax.legend()
                            ...
                            if img_path:
                                pdf.ln(10)
                                pdf.add_image(img_path, w=180, caption="Évolution des amortissements et valeurs nettes")
                    except Exception as e:
                        logger.error(f"Erreur graphique amortissements: {e}")
                        pdf.chapter_body(f"Erreur graphique amortissements: {ascii_only(str(e))}")
                else:
                    pdf.chapter_body("Aucune donnée d'amortissement disponible pour créer le résumé.")

        # Amortissements détaillés
        if "Amortissement Détaillé" in sections and has_data("detailed_amortization"):
            pdf.add_page()
            ...
            # Traiter à la fois un DataFrame ou une liste de dictionnaires
            if isinstance(detailed_amortization, pd.DataFrame) and not detailed_amortization.empty:
                try:
                    ...
                    table_data = []
                    for idx, row in detailed_amortization.head(15).iterrows():  # Limiter à 15 lignes pour le PDF
                        table_data.append([str(val) for val in row])
                    pdf.add_table(headers, table_data)
                except Exception as e:
                ...
                try:
                    # Créer un tableau formaté à partir de la liste de dictionnaires
                    headers = ["Immobilisation", "Montant", "Durée", "Taux", "Amort. N", "Amort. N+1", "Amort. N+2"]
                    table_data = []
                    ...
                    # Ajouter une ligne de total si des données ont été trouvées
                    if table_data:
                        table_data.append([
                            "TOTAL",
                            f"{total_amount:,.2f}",
                        ...
                        # Ajouter un graphique de répartition des amortissements
                        try:
                            immobilisations = []
                            amounts = []
                            ...
                            if immobilisations and amounts:  # Vérification pour éviter les erreurs avec graphique vide
                                fig, ax = plt.subplots(figsize=(8, 6))
                                ax.pie(amounts, labels=immobilisations, autopct='%1.1f%%')
                                ax.axis('equal')
                                plt.title("Répartition des immobilisations par montant")
                                ...
                                # Graphique d'évolution des amortissements par année
                                fig, ax = plt.subplots(figsize=(8, 5))
                                years = ["N", "N+1", "N+2"]
                                ...
                                ax.set_xlabel('Années')
                                ax.set_ylabel('Montant (DHS)')
                                ax.set_title('Évolution des amortissements par année')
                                
                                # Ajout légende seulement s'il y a des données
                        ...
                        pdf.chapter_body("Aucune donnée d'amortissement valide trouvée.")
                except Exception as e:
                    logger.error(f"Erreur traitement données d'amortissement: {e}")
                    pdf.chapter_body(f"Erreur traitement données d'amortissement: {ascii_only(str(e))}")
            else:
                pdf.chapter_body("Aucune donnee d'amortissements detailles disponible.")
            
            # Assurer que les tableaux d'amortissement détaillé sont inclus
            if has_data("amortization_tables"):
                pdf.add_page()
                pdf.chapter_title("Tableaux d'Amortissement Détaillés")
                
                amort_tables = st.session_state.amortization_tables
                if not isinstance(amort_tables, dict) or not amort_tables:
                    pdf.chapter_body("Aucun tableau d'amortissement détaillé disponible.")
                else:
                    for i, (asset_name, table) in enumerate(amort_tables.items()):
                        ...
                        # Ajouter un graphique d'évolution pour chaque actif
                        try:
                            years = []
                            values = []
                            
                            if isinstance(table, pd.DataFrame):
                                # Chercher les colonnes Année et Amortissement
                                year_col = None
                                amort_col = None
                                
                                for col in table.columns:
                                    col_lower = col.lower()
                                    if any(year_key in col_lower for year_key in ['année', 'annee', 'year', 'an']):
                                        year_col = col
                                ...
                                # Chercher les clés d'année et d'amortissement
                                for row in table:
                                    year_val = None
                                    amort_val = None
                                    ...
                                    for k, v in row.items():
                                        k_lower = k.lower()
                                        if any(year_key in k_lower for year_key in ['année', 'annee', 'year', 'an']):
                                            year_val = v
                            ...
                            if years and values and len(years) == len(values):
                                ...
                                ax.set_title(f'Évolution de l\'amortissement - {asset_name}')
                                
                                img_path = save_figure_safely(fig, f"amort_detail_{i}.png", temp_dir)
                                if img_path:
                                    pdf.add_image(img_path, w=180, caption=f"Évolution de l'amortissement - {asset_name}")
                        except Exception as e:
                            logger.error(f"Erreur graphique amortissement détaillé: {e}")
                            pdf.chapter_body(f"Erreur graphique amortissement détaillé: {ascii_only(str(e))}")
            ...
            try:
                if isinstance(detailed_amortization, list) and detailed_amortization:
                    # 1. Graphique de répartition des immobilisations par montant
                    immobilisations = []
                    amounts = []
                    ...
                    if immobilisations and amounts:
                        # Limiter à 7 items maximum pour lisibilité
                    ...
                    if immobilisations:
                        ...
                        # Limiter à 6 items pour lisibilité
                    ...
                    # 3. Graphique en barres des amortissements totaux par année
                    try:
                        ...
                        if img_path:
                            pdf.add_image(img_path, w=180, caption="Amortissements totaux par année")
                    except Exception as e:
        ...
        if "Tableau de Trésorerie Mensuel" in sections and has_data("monthly_cashflow_data"):
            ...
            if isinstance(monthly_data, dict):
                try:
                    ...
                    def safe_sum(dict_obj):
                        """Calcule la somme des valeurs d'un dictionnaire de façon sécurisée."""
                    ...
                    # Créer un tableau consolidé pour tous les mois
                    ...
                    # Lignes pour les dépenses
                    charges_total = safe_sum(monthly_data.get("charges_exploitation", {}))
                    consolidated_data.append(["Dépenses totales"] + [f"{charges_total:,.2f}"] * num_months)
                    ...
                    consolidated_data.append(["Solde cumulé"] + [f"{soldes[i+1]:,.2f}" for i in range(num_months)])
                    
                    # Ajouter le tableau consolidé
                    pdf.add_table(consolidated_headers, consolidated_data)
                    pdf.ln(10)
                    
                    # Détail par catégorie
                    pdf.set_font("Arial", "B", 10)
                    pdf.cell(0, 8, ascii_only("Détail des flux de trésorerie"), 0, 1, "L")
                    ...
                    if img_path:
                        pdf.ln(10)
                        pdf.add_image(img_path, w=180, caption="Évolution du solde de trésorerie mensuel")
                    
                    # Graphique de répartition des recettes et dépenses par mois
                    fig, ax = plt.subplots(figsize=(10, 6))
                    ...
                    rects2 = ax.bar(x + width/2, depenses, width, label='Dépenses')
                    ...
                    if img_path:
                        pdf.ln(10)
                        pdf.add_image(img_path, w=180, caption="Recettes et dépenses mensuelles")
            ...
            # Amélioration du tableau de trésorerie mensuel
            if has_data("monthly_treasury"):
                pdf.add_page()
                pdf.chapter_title("Détail de la Trésorerie Mensuelle")
                
                monthly_treasury = st.session_state.monthly_treasury
                
                # Afficher le tableau de trésorerie mensuelle détaillé
                if isinstance(monthly_treasury, pd.DataFrame) and not monthly_treasury.empty:
                    try:
                        ...
                        logger.error(f"Erreur affichage DataFrame trésorerie: {e}")
                        pdf.chapter_body(f"Erreur affichage tableau trésorerie: {ascii_only(str(e))}")
                        ...
                        for month, data in sorted(monthly_treasury.items()):
                            ...
                            if isinstance(data, dict):
                                ...
                                try:
                                    ...
                                    logger.error(f"Erreur calcul total trésorerie: {e}")
                                
                                pdf.add_table(headers, table_data, [120, 60])
                                pdf.ln(5)
                    except Exception as e:
                        logger.error(f"Erreur traitement dictionnaire trésorerie: {e}")
                        pdf.chapter_body(f"Erreur traitement données trésorerie: {ascii_only(str(e))}")
                
                # Ajouter des graphiques d'évolution par catégorie
                try:
                    if isinstance(monthly_treasury, pd.DataFrame) and not monthly_treasury.empty:
                        # Graphique d'évolution du solde sur tous les mois
                        mois_col = None
                        solde_col = None
                        ...
                        if mois_col is not None and solde_col is not None:
                            ...
                            ax.set_xlabel('Mois')
                            ax.set_ylabel('Solde (DHS)')
                            ax.set_title('Évolution du solde de trésorerie mensuel')
                            ax.axhline(y=0, color='r', linestyle='--', alpha=0.3)
                            ax.grid(True, linestyle='--', alpha=0.7)
                        ...
                        try:
                            ...
                            for month in months:
                                ...
                                if isinstance(data, dict):
                                    if 'Solde' in data:
                                        ...
                                        # Essayer de calculer le solde en faisant recettes - dépenses
                                        recettes = 0
                                        depenses = 0
                                        for key, value in data.items():
                                            if isinstance(value, (int, float, str)):
                                                try:
                                                    ...
                                                    if 'recette' in key.lower():
                                                        recettes += val
                                                    elif 'depense' in key.lower() or 'dépense' in key.lower():
                                                        depenses += val
                                                except (ValueError, TypeError):
                                                    pass
                            ...
                            if months and soldes and len(months) == len(soldes):
                                ...
                                ax.set_xlabel('Mois')
                                ax.set_ylabel('Solde (DHS)')
                                ax.set_title('Soldes de trésorerie par mois')
                                ax.axhline(y=0, color='r', linestyle='--', alpha=0.3)
                                ax.grid(axis='y', linestyle='--', alpha=0.7)
                                ...
                                if img_path:
                                    pdf.ln(10)
                                    pdf.add_image(img_path, w=180, caption="Soldes de trésorerie par mois")
                        except Exception as e:
                            logger.error(f"Erreur graphique soldes mensuels: {e}")
                except Exception as e:
                    logger.error(f"Erreur graphiques trésorerie mensuelle: {e}")
                    pdf.chapter_body(f"Erreur graphiques trésorerie mensuelle: {ascii_only(str(e))}")
        ...
        if "Budget TVA" in sections and has_data("vat_budget_data"):
            ...
            if isinstance(vat_data, dict):
                # Tableau récapitulatif global de la TVA
                pdf.set_font("Arial", "B", 10)
                pdf.cell(0, 8, ascii_only("Récapitulatif Global de la TVA"), 0, 1, "L")
                ...
                try:
                    ...
                    recap_tva = [
                        ["TVA Collectée (sur ventes)", f"{tva_collectee:,.2f}"],
                        ["TVA Déductible sur achats", f"{tva_deductible_achats:,.2f}"],
                        ["TVA Déductible sur immobilisations", f"{tva_deductible_immo:,.2f}"],
                    ...
                    # 2. Graphique en camembert de répartition de la TVA déductible
                    if tva_deductible_achats > 0 or tva_deductible_immo > 0:
                        ...
                        if non_zero_values:
                            ...
                            ax.axis('equal')
                            plt.title("Répartition de la TVA Déductible")
                            
                            img_path = save_figure_safely(fig, "tva_deductible_pie.png", temp_dir)
                            if img_path:
                                pdf.add_image(img_path, w=180, caption="Répartition de la TVA déductible")
                    
                    # 3. Projection de la TVA sur 12 mois (avec une légère variation)
                    fig, ax = plt.subplots(figsize=(10, 6))
                    months = range(1, 13)
                    
                    # Ajouter une légère variation pour rendre le graphique plus réaliste
                    import random
                    random.seed(123)  # Pour des résultats reproductibles
                    ...
                    if img_path:
                        pdf.ln(10)
                        pdf.add_image(img_path, w=180, caption="Projection détaillée de la TVA nette due sur 12 mois")
                    
                    # Traiter chaque section du budget TVA en détail
                    pdf.add_page()
                    pdf.chapter_title("Détail du Budget TVA")
                    ...
                    for section in sections_vat:
                        if section in vat_data and vat_data[section]:
                            ...
                            # Créer un tableau pour la section
                            headers = ["Élément", "Montant (DHS)"]
                            table_data = []
        ...
        # 5. Générer des graphiques supplémentaires
        ...
        # Ajouter une section dédiée aux graphiques s'il y en a
        if unique_charts:
            pdf.add_page()
            pdf.chapter_title("Graphiques Complets")
            pdf.chapter_body("Cette section présente tous les graphiques disponibles dans l'application.")
            ...
            for chart in unique_charts:
                try:
                    ...
                    # Vérifier si le fichier existe et n'est pas vide avant de l'ajouter
...
Contactez le support technique avec ces informations pour résoudre le problème.
        ...
        return output_file
# Fonction pour ajouter les widgets d'export à la sidebar
